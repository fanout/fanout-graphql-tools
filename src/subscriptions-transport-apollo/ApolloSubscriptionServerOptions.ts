 ...
 * This is pulled from constructor in ApolloServerBase.
 */
export const createApolloSubscriptionsOptions = (
  subscriptions: ApolloServerConfig["subscriptions"],
  /** apolloServer.graphqlPath */
  graphqlPath: ApolloServerBase["graphqlPath"],
): SubscriptionServerOptions => {
 ...
 */
export const ApolloSubscriptionServerOptions = (
  apolloServer: ApolloServerBase,
  apolloConfig: ApolloServerConfig,
  schema: GraphQLSchema,
): ISubscriptionServerOptions => {
  const {
    onDisconnect,
    onConnect,
    keepAlive,
  ...
  const apolloSubscriptionServerOptions: ISubscriptionServerOptions = {
    execute,
    keepAlive,
    onConnect: onConnect
      ? onConnect
  ...
  return apolloSubscriptionServerOptions;
};
